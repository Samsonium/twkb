...
export default class Struct {
    ...
    private actualByte: [number, number]; // [byte, bit-n]
     ...
     */
    public parse(stream: ByteReader, def: StructDef = this.description): Record<string, any> {
        ...
        for (const key in def) {
            if (!Array.isArray(def[key]))
                ...
                for (let i = 0; i < length; i++) {
                    if (!this.actualByte)
                        this.actualByte = [stream.readByte(), 0];
                ...
                const resValue = def[key][0](data);

                if (def[key][2] && typeof def[key][2] == 'function')
                    result[key] = def[key][2](resValue)
